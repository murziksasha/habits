...
/** Catalog with optional progress chips when logged in. */
export default function CoursesPage() {
  ...
  useEffect(() => {
    ...
    void api<{ progress?: ProgressRow[] }>("/me/home", { token })
  ...
  return (
    <div className="space-y-8 pb-20 md:pb-0">
      <div className="flex flex-wrap items-end justify-between gap-3">
        ...
        <Link href="/learn" className="btn-secondary !py-2 text-sm min-h-11">
          {t.nav.learn} →
        </Link>
      </div>

      {user && <ContinueCta variant="card" />}
        ...
        <section key={group} className="space-y-3">
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {courses.map((slug) => {
              ...
              const p = progress.find((x) => x.slug === slug);
              return (
                <Link
                  key={slug}
                  href={href}
                  className="card min-h-11 hover:scale-[1.01] transition border-l-4"
                  ...
                  <div className="flex items-start justify-between gap-2">
                    ...
                    <div className="flex flex-wrap gap-1 justify-end">
                      ...
                      {p && p.completedLessons > 0 && (
                        <span className="rounded-full bg-brand-soft px-2 py-0.5 text-[10px] font-black text-brand-dark">
