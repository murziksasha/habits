...
export function DashboardClient() {
  ...
  useEffect(() => {
    ...
    }>("/me/login-bonus", { method: "POST", token })
      .then(async (d) => {
        if (d.character) setCharacter(d.character);
        if (!d.alreadyClaimed && (d.rewardXp ?? 0) > 0) {
          setLoginBonus(d.rewardXp ?? 0);
          await refresh();
        }
      })
      .catch(() => undefined);
    ...
    persona === "parent"
      ...
      : persona === "teacher"
        ...
        : [
            ...
            { href: "/profile", icon: "⭐", label: locale === "en" ? "Build" : "Прокачка" },
          ];
  ...
  return (
    <div className="space-y-6">
      ...
      {loginBonus != null && loginBonus > 0 && (
        <div className="card border-sun/40 bg-sun/10 text-sm font-black">
          🎁 {locale === "en" ? "Daily check-in" : "Щоденний бонус"}: +{loginBonus} XP
        </div>
      )}
          ...
          <div>
            ...
            <p className="text-sm text-ink-muted flex flex-wrap items-center gap-2">
              ...
              {(character?.progression?.skillPoints ?? 0) > 0 ? (
                <Link
                  href="/profile"
                  className="rounded-full bg-grape/15 px-2 py-0.5 text-xs font-black text-grape hover:underline"
                >
                  ⭐{" "}
                  {locale === "en" ? "Skill points" : "Очки навичок"}:{" "}
