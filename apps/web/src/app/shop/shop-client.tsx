...
export function ShopClient() {
  ...
  async function buy(itemId: string) {
    ...
    try {
      const d = await api<{
        ...
        loot?: { labelUk?: string; labelEn?: string; icon?: string } | null;
      }>("/shop/buy", { method: "POST", token, body: { itemId } });
      if (d.character) setCharacter(d.character);
      await refresh();
      await load();
      if (d.loot) {
        setMsg(
          `${d.loot.icon ?? "🎁"} ${locale === "en" ? d.loot.labelEn : d.loot.labelUk}`,
        );
      } else {
  ...
  function labelFor(id: string) {
    ...
    if (id === "mystery_crate") return "🎁 Mystery crate";
    if (id === "skill_point_1") return "⭐ Skill point ×1";
    if (id === "skill_point_3") return "⭐ Skill points ×3";
